	...
	if( modo == 2 ) 	// Modo escritura.
	{
		...
		while(contador <= paquete_ultimo)
		{
			...
			if(contador < numero_paquetes)
			{
				...
				{
					bloque_data[contadorA]= fgetc(archivo); 	// Paquetes de 512
				}
			}
			else
			{
				int contadorA=0;
				for(contadorA=0; tamanho_archivo -((contador-1)*512);contadorA++)
				{
					bloque_data[contadorA]= fgetc(archivo); 	// Paquetes de 512
				}
			}
			...
			//int aux_;
			//for(aux_=0; aux_ < 512;aux_++){mensaje_out[aux_+4] = bloque_data[aux_];}
			char *mensaje_auxiliar;
			mensaje_auxiliar = mensaje_out + 4;
			strcpy(mensaje_auxiliar, bloque_data);
			mensaje_auxiliar += strlen(bloque_data);
			printf("%s\n",mensaje_auxiliar);
			printf("|%i|%i|\n|%i|%i|\n",mensaje_out[1],mensaje_out[3], mensaje_in[1],mensaje_in[3]);
			printf("-1-------------%s--------------\n",mensaje_out);
			printf("Enviando paquete DATA No.%i\n",paquete_ultimo);
			if((sendto(descriptor_socket,mensaje_out,mensaje_auxiliar-mensaje_out,0,(struct sockaddr *)&direccion_servidor,sizeof(struct sockaddr)))<0)
			{
				perror("sendto()");
				exit(EXIT_FAILURE);
			}
