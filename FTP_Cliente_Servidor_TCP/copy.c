...
int main(int argc, char * argv[])
{
	...
	{	
		...
		do
		{
			...
			char mensaje[4];
			mensaje_out[0] = ACK/256;
			mensaje_out[1] = ACK%256;
			mensaje_out[2] = paquete_ultimo/256;//paquete_ultimo/256;
			mensaje_out[3] = paquete_ultimo%256;//paquete_ultimo%256;
			//mensaje_out[4] = 0/256;
			//aux=mensaje + 4;
			//Enviamos el paquete ACK
			if((sendto(descriptor_socket,mensaje_out,5,0,(struct sockaddr *)&direccion_servidor, sizeof(struct sockaddr)))<0)
			{
				perror("sendto()");
				exit(EXIT_FAILURE);
			}
			if(paquete_esperado == paquete_recibido)
			{	
			...
			paquete_recibido = mensaje_in[2]/256;
			paquete_recibido = mensaje_in[3]%256;
			int paquete_enviado=0;
			paquete_enviado = mensaje[2]/256;
			paquete_enviado = mensaje[3]%256;

			FILE *tmp;
			tmp = fopen("log","a");
			printf("Enviado:   | %i | %i |\n",ACK, paquete_enviado);
			printf("Recibido:  | %i | %i |\n",codigo_op, paquete_recibido,mensaje_in);
			fwrite(strcpy(mensaje_out,""),1,5,tmp);
			/*char mensaje_[4];
			mensaje_[0]=mensaje_in[0];
			mensaje_[1]=mensaje_in[1];
			mensaje_[2]=mensaje_in[2];
			mensaje_[3]=mensaje_in[3];
			*/
			fwrite(strcpy(mensaje_in,""),1,size_,tmp);
			fclose(tmp);

			//printf("Codigo Operacion de paquete recibido %i\n",paquete_recibido);
		}while(size_ == 512);
		fclose(archivo);
